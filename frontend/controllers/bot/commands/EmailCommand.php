        ...
        switch ($state) {
            ...
            case 2:
                ...
                return (new PostCommand($this->telegram,
                    new Update(json_decode($this->update->toJson(), true))))->execute(true, 'Чтобы разместить пост во всех соцсетях, просто пришлите боту текст сообщения, фото или видео – после этого бот спросит Вас опубликовать немедленно или в заданное время. Без Вашего подтверждения ничего публиковаться не будет.

Если вы хотите опубликовать пост и с текстом и с фото/видео – просто пришлите что-то одно сначала, после этого у вас появятся кнопки для добавления текста и фото/видео.

Чтобы настроить соцсети, в которые будет публиковаться Ваши посты – зайдите в свой личный кабинет на сайте по ссылке: http://app.slavabot.ru и привяжите аккаунты.
');

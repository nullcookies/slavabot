...
use frontend\controllers\bot\libs\VkException;


class Vk {

    private $v = '5.68'; // версия Api VKontakte @link https://vk.com/dev/versions
    ...
    private function call($url = ''){
        ...
        if(isset($json['error'], $json['error']['error_msg'], $json['error']['error_code'])){
            ...
            throw new \frontend\controllers\bot\libs\VkException($json['error']['error_msg'], $json['error']['error_code']);
        }
    ...
    */
    ...
    {
        return $this->api('notifications.get', $vars);
    }

    public function getUsers($vars = [])
    {
        return $this->api('users.get', $vars);
    }
}
